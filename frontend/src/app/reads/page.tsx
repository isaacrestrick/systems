...
function CacheDemo() {
  ...
  const compareBursts = async () => {
    ...
    addLog("Burst: 10 reads WITHOUT cache...");
    ...
    await fetch(`${API_BASE}/cache/clear`, { method: "POST" });
    addLog("Burst: 10 reads WITH cache (same keys)...");
...
function ReplicaDemo() {
  ...
  const burstReads = async () => {
    setRunning(true);
    addLog("Burst: 20 reads across replicas...");
...
function StampedeDemo() {
  ...
  const triggerStampede = async (protected_: boolean) => {
    ...
    addLog(`Sending 20 concurrent requests (${protected_ ? "protected" : "unprotected"})...`);
  ...
  return (
    <div className="space-y-4">
      <header className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Scaling Reads</h1>
